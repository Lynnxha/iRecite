...
const NewLink = ({
  ...
  return (
    <Link
      href={href}
      className="w-fit py-2 px-3 bg-white rounded-full text-gray-700 text-sm hover:text-gray-500"
    >
      {children}
    </Link>
  );
...
export default function Header({ data }: { data: SurahInfo[] }) {
  ...
  return (
    <header className=" bg-gradient-to-br dark:from-sec-color-dark dark:to-pri-color-dark from-pri-color-light to-sec-color-light flex-col p-2 pt-14 pb-8 flex">
      <div className="p-2 md:px-8">
        <div className="text-center">
          <div className="m-auto my-6 w-36 p-8">
            <Image
              src={`${
                settings.theme ? "/logo/logo-192x.png" : "/logo/logo-192x.png"
              }`}
              alt="i-Recite logo"
              width={1000}
              height={1000}
