...
export default function Navbar({
  ...
  return (
    <div
      className={`fixed w-full z-40 transition-all duration-100 ${
        isActive ? "top-0" : "-top-12"
      } ${!surahInfo && "dark:drop-shadow-md"}`}
    >
      <div className="flex justify-between items-center px-5 sm:px-8 h-12 bg-white dark:bg-pri-color-dark">
        <Link href={"/"}>
          <h1 className="text-xl font-bold">i-Recite</h1>
        </Link>
        ...
        <>
          <div className="overflow-x-hidden w-screen">
            <div className="px-5 sm:px-8 py-1 bg-white dark:bg-pri-color-dark drop-shadow-[0_3px_0px_rgba(0,0,0,0.1)]">
              <div
                className="w-fit flex items-center gap-1 cursor-pointer font-semibold"
                onClick={() => setSurahNav(!surahNav)}
              >
            ...
            >
              <div
                ref={ref}
                className="z-[-1] absolute rounded-md p-2 w-fit flex bg-white translate-y-2 left-4 right-4 sm:left-8 dark:bg-sec-color-dark drop-shadow-md"
              >
